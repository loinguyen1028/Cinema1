...
class MovieDialog(tk.Toplevel):
    def __init__(self, parent, controller, mode="add", movie_id=None, on_success=None):
        ...
        self.on_success = on_success  # Hàm callback để refresh bảng bên ngoài
    ...
    def render_ui(self):
        ...
        self.e_name = self.create_input(row1, "Tên phim", self.movie_data["name"], side=tk.LEFT)
        ...
        txt_frame = tk.Frame(f_desc, bg="white", bd=1, relief="solid")  # Viền cho đẹp
        ...
        # Text Widget
        self.txt_desc = tk.Text(txt_frame, font=("Arial", 11), height=8, relief="flat",
        ...
        tk.Button(right_col, text="Lưu", bg="#1976d2", fg="white", font=("Arial", 11, "bold"),
                  width=15, relief="flat", command=self.save_action).pack(side=tk.BOTTOM, pady=10)
    ...
    def save_action(self):
        ...
        if success:
            ...
            if self.on_success: self.on_success()  # Refresh bảng cha
            self.destroy()
        else:
